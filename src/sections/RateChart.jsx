	...
	return (
		<section className="rates" style={{paddingTop:0}}>
			<div className="rates-container">
				<div className="rates-card reveal" style={{marginTop:16}}>
					<div className="rates-header" style={{marginBottom:8, alignItems:'baseline'}}>
						<h3 className="rates-title" style={{margin:0}}>7‑Day USD→NGN Trend</h3>
						<div style={{fontSize:12, color:'#DBEAFE'}}>Synthetic series; switch to real history later</div>
					</div>
					{error && (<div style={{color:'#fee2e2', background:'#7f1d1d', padding:8, borderRadius:8}}>{error}</div>)}
					{view && (
							...
							<svg ref={svgRef} width={view.W} height={view.H} viewBox={`0 0 ${view.W} ${view.H}`} style={{display:'block'}} onMouseMove={onMove} onMouseLeave={onLeave}>
								<defs>
									<linearGradient id="rc-fill" x1="0" y1="0" x2="0" y2="1">
										<stop offset="0%" stopColor="#FFB806" stopOpacity="0.28" />
										<stop offset="100%" stopColor="#FFB806" stopOpacity="0" />
									</linearGradient>
								</defs>
								<g stroke="#ffffff22" strokeWidth="1">
									<line x1={view.padL} x2={view.W-view.padR} y1={view.H-view.padB} y2={view.H-view.padB} />
									<line x1={view.padL} x2={view.padL} y1={view.padT} y2={view.H-view.padB} />
									{[0,0.25,0.5,0.75,1].map((t,i)=>{
										...
										return (
											<g key={i}>
												...
												<text x={view.padL-8} y={y+4} textAnchor="end" fontSize="11" fill="#DBEAFE">₦{v.toLocaleString()}</text>
											</g>
										)
									})}
									{data.map((p,i)=> (
										<text key={i} x={view.points[i].x} y={view.H-14} textAnchor="middle" fontSize="11" fill="#DBEAFE">{p.label}</text>
									))}
								</g>
								<path d={`${view.path} L ${view.W-view.padR},${view.H-view.padB} L ${view.padL},${view.H-view.padB} Z`} fill="url(#rc-fill)" />
								<path d={view.path} fill="none" stroke="#FFB806" strokeWidth="3" />
								{hover && (
									<g>
										<line x1={hover.x} x2={hover.x} y1={view.padT} y2={view.H-view.padB} stroke="#ffffff55" />
										<circle cx={hover.x} cy={hover.y} r="4.5" fill="#FFB806" stroke="#fff" strokeWidth="1.5" />
										<rect x={Math.min(hover.x+8, view.W-170)} y={view.padT+8} width="160" height="48" rx="8" fill="#0b274a" stroke="#ffffff22" />
										<text x={Math.min(hover.x+16, view.W-162)} y={view.padT+28} fontSize="12" fill="#DBEAFE">{data[hover.idx].label}</text>
										<text x={Math.min(hover.x+16, view.W-162)} y={view.padT+44} fontSize="14" fill="#fff" fontWeight="700">₦{hover.v.toLocaleString()}</text>
									</g>
								)}
							</svg>
						</div>
